...
class OverviewCallWidget extends StatelessWidget{
  final String headerTitle,subTitle;
  final int index;
  OverviewCallWidget({required this.index,required this.subTitle,required this.headerTitle});
  @override
  Widget build(BuildContext context) {
    BlocProvider.of<ScheduledCallsDashboardBloc>(context).selectedFiter=3;
              ...
              BlocBuilder<ScheduledCallsDashboardBloc,ScheduledCallsDashboardState>(
                  builder: (context,state){
                    if (state is AllCallsDashboardMadeState) {
                      if (state.callsMadeCount==0) return NoDataFound();
                      return Center(child: Text(state.callsMadeCount.toString()));
                    }
                    return Container();
                  }),
            ],
